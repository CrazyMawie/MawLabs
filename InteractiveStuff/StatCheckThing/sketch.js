...
let synergyBonusScale = 0.075;
let synergyScaleDisplay = 0.075;
...
function setup() {
	...
	// Synergy Bonus Controls
	{
		...
		let relY_dec_bonus = relY_base - 10;
		...
		let relY_bonus_display = relY_base - 15;
		...
		let relY_inc_bonus = relY_base - 10;
		...
		let decS = createButton('-');
		let relX_decS = relX_base + 70;
		let relY_decS = relY_base + 26;
		decS.position(relX_decS + canvasOffsetX, relY_decS + canvasOffsetY);
		uiElements.push({el: decS, relX: relX_decS, relY: relY_decS});
		decS.mousePressed(() => changeSynergyScale(-0.025));

		synergyScaleDisplay = createDiv(String((synergyBonusScale * 100) + '%'));
		let relX_redScale = relX_base + 94;
		let relY_redScale = relY_base + 26;
		synergyScaleDisplay.position(relX_redScale + canvasOffsetX, relY_redScale + canvasOffsetY);
		uiElements.push({el: synergyScaleDisplay, relX: relX_redScale, relY: relY_redScale});
		synergyScaleDisplay.style('font-family', 'sans-serif');
		synergyScaleDisplay.style('font-size', '20px');
		synergyScaleDisplay.style('color', 'white')

		let incS = createButton('+');
		let relX_incS = relX_base + 150;
		let relY_incS = relY_base + 26;
		incS.position(relX_incS + canvasOffsetX, relY_incS + canvasOffsetY);
		uiElements.push({el: incS, relX: relX_incS, relY: relY_incS});
		incS.mousePressed(() => changeSynergyScale(0.025));
	}
}
...
function draw() {
	...
	if (complicationEnabled) {
		...
		let yellowBonusScale = 1.0 + (yellowBonus * synergyBonusScale);
...
// Change Complication Scale
function changeSynergyScale(delta) {
	if (ballInMotion || displayEndScreen) {
	...
	synergyBonusScale = constrain(synergyBonusScale + delta, 0.075, .375);

	let forDisplayScale = round((synergyBonusScale * 100), 1);

	if (synergyScaleDisplay) synergyScaleDisplay.html(String(forDisplayScale + '%'));
		}
}
